...
//
// The gococ.cli.RequestResolver allows you to specify global level flags. This command
...
//
// Like the gococ.cli.RequestResolver, this will place the parsed params directly into the
...
func ParseArgs(cxt gococ.Context, params *gococ.Params) (interface{}, gococ.Interrupt) {
...
func ShowHelp(cxt gococ.Context, params *gococ.Params) (interface{}, gococ.Interrupt) {
	showHelp := false
...
func RunSubcommand(cxt gococ.Context, params *gococ.Params) (interface{}, gococ.Interrupt) {
	...
	if len(stoplist) > 0 {
		for _, stop := range stoplist {
			if stop == route {
				return nil, &gococ.FatalError{"Illegal route."}
			}
		}
	}

	return nil, &gococ.Reroute{route}
}
...
func ShiftArgs(c gococ.Context, params *gococ.Params) (interface{}, gococ.Interrupt) {
	...
	if !ok {
		return nil, &gococ.FatalError{"Could not get arg out of context: No such arg name."}
	}

	if len(args) < n {
		c.Put(argName, make([]string, 0))
		//log.Printf("Not enough args in %s", argName)
		return nil, &gococ.RecoverableError{"Not enough arguments."}
	}
	targetArg := n - 1

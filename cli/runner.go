...
//
// By default, the `gococ.BasicRequestResolver` is used for resolving request.
...
func New(reg *gococ.Registry, router *gococ.Router, cxt gococ.Context) *Runner {
...
func (r *Runner) Subcommand(name, summary, usage string, flags *flag.FlagSet) *gococ.Registry {
	if flags == nil {
...
func subcommandHelp(reg *gococ.Registry) string {
	...
	for _, name := range names {
		...
		help := fmt.Sprintf("\t%s: %s", name, gococ.RouteDetails(rs).Description())

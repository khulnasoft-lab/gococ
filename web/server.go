...
func Serve(reg *gococ.Registry, router *gococ.Router, cxt gococ.Context) {
	...
	handler := NewGococHandler(reg, router, cxt)

	// MPB: I dont think there's any real point in having a multiplexer in
	// this particular case. The Gococ handler is mux enough.
	//
	// Note that we can always use Gococ with the built-in multiplexer. It
	// just doesn't make sense if Gococ's the only handler on the app.
	//http.Handle("/", handler)
...
func ServeTLS(reg *gococ.Registry, router *gococ.Router, cxt gococ.Context, certFile, keyFile string) {
	...
	server := &http.Server{Addr: addr}
	server.Handler = NewGococHandler(reg, router, cxt)
...
func handleSignals(router *gococ.Router, cxt gococ.Context, server *http.Server) {
...
func shutdown(router *gococ.Router, cxt gococ.Context) {
...
//   - http.ResponseWriter: The response writer.
//   - server.Address: The server's address and port (NOT ALWAYS PRESENT)
func NewGococHandler(reg *gococ.Registry, router *gococ.Router, cxt gococ.Context) *GococHandler {
	handler := new(GococHandler)
...
func (h *GococHandler) addDatasources(cxt gococ.Context, req *http.Request) {
	parsedURL := req.URL
...
func (h *GococHandler) ServeHTTP(res http.ResponseWriter, req *http.Request) {
	...
	defer func() {
		...
		if err := recover(); err != nil {
			...
			h.BaseContext.Logf("error", "GococHandler trapped a panic on route '%s' in command '%s': %v", rname, where, err)

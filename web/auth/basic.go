 ...
 * 	  the current chain.
 */
func Basic(c gococ.Context, p *gococ.Params) (interface{}, gococ.Interrupt) {
...
func parseBasicString(header string) (user, pass string, err error) {
	...
	pass = ""
	if len(parts) < 2 {
		err = &gococ.RecoverableError{"No auth string found."}
		return
	}
...
func sendUnauthorized(realm string, res http.ResponseWriter) (interface{}, gococ.Interrupt) {
	// Send a 403
